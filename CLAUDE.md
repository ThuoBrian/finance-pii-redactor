...
- **Sharing the tool:** distribution is via GitHub's **Download ZIP** button on the repo (or `git clone`), followed by `run.bat`/`run.sh`. Updating is a fresh ZIP download (or `git pull`) over the existing folder; unlike the old installers, nothing preserves a local `data/Names List - Organized.xlsx` automatically, so back it up first if you keep one locally rather than on a shared drive (see `data/README.md`). (The one-line `install.ps1`/`install.sh` installers - which downloaded the source, preserved the local master list, and launched the app in one step - were removed as unused; the older `package.sh`/`package.bat`/`package.ps1` zip-builder scripts were removed earlier for the same reason.)
