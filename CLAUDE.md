...
- **Building a shareable package:** `package.sh` (macOS/Linux) and `package.bat` / `package.ps1` (Windows) produce `../finance-pii-redactor.zip` — a lightweight **source** zip (no `.venv`, no model; excludes caches and `.claude`). The recipient extracts it and runs `run.bat`/`run.sh`, whose first run builds the env (the pinned spaCy model installs as part of `uv sync`). The Windows script uses built-in `robocopy` + `Compress-Archive` only (no `zip` CLI needed).
