...
### The crosswalk's fuzzy "Possible match" suggests a name from the wrong category
- **Symptom:** A flagged auto-id's "Possible match" hint points at a curated name that is obviously the wrong kind of entity — e.g. a document's unmatched vendor string suggests a *Funder*'s name, or vice versa.
- **Cause:** `Pseudonymizer`'s fuzzy-match candidate pool (`build_candidate_index`) is grouped by `entity_type` only, not `category`, because the flagged name's own category is unknown by definition (that's why it needs a suggestion). Vendor and Funder both detect as `ORGANIZATION`, so a Vendor typo can score closest against an unrelated Funder name that happens to be textually similar — the same confusion `quality_report()`'s `cross_category_duplicate` check flags for *exact* matches, but for a near match there is no such guard.
- **Solution:** This is not silently wrong — `Assignment.suggested_category` (added alongside `suggested_name`/`suggested_pseudonym`/`suggested_score`) carries the *matched* entry's own category, and `crosswalk_dataframe` renders it in the "Possible match" column (e.g. `Acme Foundation (FND-3001, Funder, 92% match)`). Always check that category before accepting a suggestion — a mismatch is a strong signal it's coincidental similarity, not a typo of the same entity, and the suggestion is advisory only regardless (it never changes which pseudonym gets assigned).

### A benign duplicate row's `display_name` flips between workbook edits
- **Symptom:** The crosswalk's "Possible match" hint shows a curated name in different casing/spacing than expected (e.g. `ACME LTD` instead of `Acme Ltd`), and it changes after reordering or re-saving the workbook even though the data didn't meaningfully change.
- **Cause:** `MasterListRepository.master_map()` builds its `(entity_type, normalized_name) -> MasterEntry` map in one pass over the curated rows. For two rows that normalize to the same key and share the same `Internal ID` (a "benign exact duplicate" — see the data-quality entry above), the loop used to overwrite the map entry with *every* matching row rather than keeping the first, so `display_name` came from whichever row pandas iterated last — order-dependent, contradicting the method's own "first row wins" doc.
- **Solution:** Fixed: the loop now skips a key already present in the map (`if key in mapping: continue`), so `display_name` (like everything else about the entry) consistently comes from the first row, regardless of how many benign duplicates follow it. The pseudonym itself was never affected — this only fixes which row's *display text* is used in the fuzzy-suggestion hint.

### Re-running many files feels slower than expected even though the master list didn't change
- **Symptom:** Processing several Excel/PDF files in one session re-pays a noticeable cost each time, even though `app.py`'s `@st.cache_resource` bundle should mean the master list is loaded once.
- **Cause:** Each `RedactExcelService.redact` / `RedactPdfService.execute` call constructs a fresh `Pseudonymizer` per file (correct — assignments must not leak across files), and `Pseudonymizer.__init__` used to always rebuild its fuzzy-match candidate index (`_candidates_by_type`) from a full pass over `master_map` — tens of thousands of entries including every alias variant — even though that index depends only on the master list, which the `@st.cache_resource` bundle had already loaded once.
- **Solution:** `build_candidate_index(master_map)` is now called once inside `app.py`'s cached bundle (alongside `master_map()` itself) and passed to `RedactExcelService`/`RedactPdfService`, which forward it to each `Pseudonymizer` they construct via the new `candidates_by_type` parameter. A `Pseudonymizer` given no index still derives one itself from `master_map` (this is what test/script callers that construct one directly get), so nothing outside the composition root needs to change.
