...
- **Cause:** This is expected, not a bug. Ad-hoc custom words (`finance_redactor/domain/custom_words.py`) are deliberately never looked up in the master list — they're a per-run, typed-in-the-box list, not curated data, so there's no `Internal ID` for them to resolve to. The code is still deterministic (the same word always hashes to the same `CST-AUTO-*` id, even across different files and sessions), and it still beats an overlapping generic model guess in `dedupe_overlapping` - it just never becomes curated. For PDF specifically, this box is the *only* redaction mechanism (there's no automatic detection at all - see the "PDF has no automatic detection" entry below), not a supplement like it is for Word.
...
### PDF has no automatic detection - only the words you type in

- **Symptom:** Uploading a PDF with real names/organizations in it doesn't redact anything unless you typed those exact words into the "Words/phrases to redact" box first; the entity-selection and confidence-threshold widgets that Excel/Word have are gone entirely from PDF's Advanced settings.
- **Cause:** This is a deliberate design decision, not a missing feature. `RedactPdfService` (`finance_redactor/application/redact_pdf.py`) no longer runs spaCy or master-list matching at all - it only searches for the literal words/phrases passed in via `custom_words` (`domain/custom_words.find_custom_words`). Excel and Word are unaffected; only PDF changed.
- **Solution:** Type or paste every word/phrase you want redacted into the box. Leaving it empty stops the flow with a message (nothing to redact) rather than silently producing an unchanged file that looks processed. See the "0 names" and "does not show up until I do something" master-list entries above - they don't apply to PDF at all anymore, since PDF never reads the master list.
...
- **Applies to:** Word and Excel only. PDF no longer runs spaCy/master-list detection at all (see "PDF has no automatic detection" above), so the model-fusing scenario below can't occur there anymore - this fix (and the rest of this entry) is now Word/Excel-specific, even though it was originally found and fixed in the PDF flow.
