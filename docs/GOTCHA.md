...
### The master-list summary shows 0 names after pointing at the shared Box folder

- **Symptom:** After setting `FPR_MASTER_LIST_DIR` and restarting the app, the master-list summary in **Advanced settings** shows 0 names for every category, but there is no error message.
- **Cause:** `MasterListRepository._parse` only catches `FileNotFoundError` and returns an empty list on it — by design, a bad path or a missing file fails silently as "0 names," not as a crash (see `finance_redactor/infrastructure/names/master_list_repository.py`). Any of the following put the app in that state:
  1. **The environment variable isn't actually set in the process that launched the app.** On Windows, `setx` only takes effect for *new* processes started after it runs — the terminal window you ran `setx` in does not see the update. If you launch by double-clicking `run.bat` from File Explorer rather than a freshly opened terminal, Explorer itself may need to restart (or you need to log off/on) before it picks up a `setx`'d value, not just "open a new terminal."
  2. **The path is wrong for this machine.** Box Drive does not guarantee the same literal path across users/computers (different drive letter, username, or folder nesting) — the value must be *your own* local path to the shared folder, not one copied verbatim from a colleague.
  3. **The filename doesn't match exactly**, or a sync-conflict copy was created — see [Keep the filename exactly as-is](../data/README.md#keep-the-filename-exactly-as-is) in `data/README.md`.
  4. **The file is an online-only Box placeholder that hasn't synced**, so it isn't actually present at that path yet. Right-click the shared folder in Box Drive and choose **Make Available Offline**.
- **Solution:** The app itself now offers an easier fix for cause 1 (by far the most common): whenever 0 names are loaded, it automatically shows a **"Set up your shared master list"** dialog (`finance_redactor/presentation/master_list_setup.py`) prompting for the folder path directly in the browser — no terminal, no `setx`, no restart. It persists the choice to a small per-user settings file (`~/.finance_pii_redactor/settings.json`, via `finance_redactor.config.save_master_list_dir`) that takes precedence over `FPR_MASTER_LIST_DIR` and applies on the next page rerun. If you'd rather still use the environment-variable route (e.g. for a scripted/team-wide deployment), that keeps working unchanged as a fallback — check causes 2-4 above, close and reopen the terminal (or restart Explorer/log off-on if launched by double-click) before relaunching `run.bat`/`run.sh`, then confirm the row counts in **Advanced settings** match what teammates on the same shared list see.
...
- **Symptom:** Uploading a PDF with real names/organizations in it doesn't redact them unless you typed those exact words into the "Additional words/phrases to redact" box first; the entity-selection and confidence-threshold widgets that Excel/Word have are gone entirely from PDF's Advanced settings. Meanwhile an email address or a website URL in the same PDF *does* get redacted automatically, with nothing typed in.
- **Cause:** This is a deliberate design decision, not a missing feature - just a narrower one than it might first look. `RedactPdfService` (`finance_redactor/application/redact_pdf.py`) never runs spaCy or master-list matching, so it can't guess at names/organizations the way Excel/Word do. It does always run a separate, spaCy-free `PatternDetector` (`infrastructure/detection/pattern_detector.py`, wrapping only Presidio's regex-based `EmailRecognizer`/`UrlRecognizer`) for emails and websites, and by default blacks out every embedded image on the page ("Also black out images / logos", checked by default, works in both redaction styles) - neither of those is a statistical guess, so neither reintroduces the unreliable-detection problem the original PDF change was solving. Excel and Word are unaffected; only PDF's detection shape changed.
- **Solution:** Emails, websites, and images need nothing typed in - they're caught automatically. Type or paste anything else you want redacted (a name, a project codename, a case number) into the box; it's a supplement, not the only mechanism. Leaving it empty no longer stops the flow - it just means nothing extra is added on top of the automatic detection. See the "0 names" and "does not show up until I do something" master-list entries above - they don't apply to PDF at all, since PDF never reads the master list.
...
- **Applies to:** Word and Excel only. PDF no longer runs spaCy/master-list detection at all (see "PDF has no automatic name/organization detection" above), so the model-fusing scenario below can't occur there anymore - this fix (and the rest of this entry) is now Word/Excel-specific, even though it was originally found and fixed in the PDF flow.
...
- **Solution:** `find_custom_words` now folds accents (via `fold_diacritics` in `finance_redactor/domain/text_folding.py`, a length-preserving Unicode NFKD decomposition, the same technique `recasing.recase_uppercase` uses for ALL-CAPS) before matching, in both directions: typing `Jose Garcia` matches `José García` in the text, and typing `José García` matches an unaccented `Jose Garcia` (e.g. from an OCR pass that dropped accents). The detection reported in the crosswalk/review table still shows the real spelling as it appeared in the document, not the folded form.

### Accented names in the master list didn't match a differently-accented document spelling (fixed)

- **Symptom:** A master-list name stored with accents (`José García`) wasn't detected/pseudonymized when a document spelled it without accents (`Jose Garcia`), or vice versa - even though the exact same name (case- and spacing-insensitive) worked fine.
- **Cause (fixed):** Two separate spots needed the same fix as the custom-words box above: `CustomNameRecognizer` (`finance_redactor/infrastructure/detection/custom_recognizer.py`) built its Aho-Corasick match keys and scan text from `.lower()` alone, so an accent difference meant the name was never even detected as PERSON/ORGANIZATION. And `pseudonyms.normalize()` (`finance_redactor/domain/pseudonyms.py`), which builds both the master-map keys and the detected-text lookup key, only did whitespace-collapse + casefold - so even a name that *was* detected some other way (e.g. spaCy) would resolve to a mismatched auto-id instead of the curated one.
- **Solution:** Both now fold accents via the same shared `fold_diacritics` helper: `CustomNameRecognizer` folds its automaton keys and the scanned text, so detection itself works across accent differences; `pseudonyms.normalize()` folds too, so the master map and the lookup key agree regardless of which side has the accents. You can now enter a name in the master list with or without accents and it will resolve correctly either way - no need to match the document's exact accented spelling.
...
### PDF text is not replaced, or a typed word/phrase isn't found
...
- **Cause 2 (Word/Excel only):** The same span was detected by both the spaCy model and the master list, or a name appears under two categories. This cannot happen in PDF anymore, since PDF has no spaCy/master-list detection to conflict with (see "PDF has no automatic name/organization detection" above) - only relevant if you're seeing this for Word or Excel.
