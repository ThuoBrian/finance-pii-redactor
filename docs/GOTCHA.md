...
- **Solution:** Point every teammate's install at **one shared, access-controlled copy** of the workbook by setting the `FPR_MASTER_LIST_DIR` environment variable to that shared location on each machine, then restart the app. On Windows, `run.bat` now asks for this on first launch, letting you pick the folder with a pop-up window — see **[Sharing one master list across a team](../data/README.md#sharing-one-master-list-across-a-team)** in `data/README.md` for the exact steps and the compliance note (the shared location must meet the same Confidential-data handling rules as a local copy). Unset (the default), behavior is unchanged — the local `data/` folder next to the code is used, as before.

### `run.bat`'s master-list folder picker doesn't seem to appear
- **Symptom:** After typing `2` at the "Where is your master list?" prompt and pressing Enter, nothing seems to happen — no window pops up.
- **Cause:** The folder-browser window (`FolderBrowserDialog`, launched via PowerShell/WinForms) sometimes opens *behind* the terminal window rather than on top of it, which reads as "it didn't open" when it actually did.
- **Solution:** Check the taskbar, or press Alt+Tab, for a new "Browse For Folder" window. Select the folder and click OK; the terminal resumes automatically once the window closes. If you genuinely want to back out, click Cancel in that window — the app falls back to the local master list rather than hanging.

### `run.bat` keeps asking for the master list location every time
- **Symptom:** The "Where is your master list?" prompt reappears on every launch instead of being remembered.
- **Cause:** The answer is saved as two Windows user-level environment variables (`FPR_MASTER_LIST_CONFIGURED`, `FPR_MASTER_LIST_DIR`), read via `[Environment]::GetEnvironmentVariable(...,'User')`. If something clears them between launches (a company policy that resets user environment variables, a different Windows user account, or running from a fresh/rolled-back VM snapshot), the prompt reappears — this is not a bug, it genuinely no longer has a saved answer.
- **Solution:** Answer the prompt again; it will persist normally afterward. To confirm what's saved: `powershell -Command "[Environment]::GetEnvironmentVariable('FPR_MASTER_LIST_DIR','User')"`. To intentionally reset and be asked again, run `reconfigure_master_list.bat`.
